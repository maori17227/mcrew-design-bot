...
\u{1F464} <b>Client:</b> ${userName} (@${userUsername})
\u{1F194} <b>User ID:</b> ${userId2}

\u{1F4CB} <b>Service:</b> ${service}
...
covers|Artist Name|Track Name`, null, BOT_TOKEN);
              ...
              await sendMessage(chatId, `\u{1F4DD} Send photo with caption:
...
${item.id}`, null, BOT_TOKEN);
              ...
              await sendMessage(chatId, `\u{1F4DD} Send audio with caption:
...
motion|Video Title`, null, BOT_TOKEN);
              ...
              await sendMessage(chatId, `\u{1F4DD} Send video with caption:
...
motion|Video Title`, null, BOT_TOKEN);
          ...
          if (text === "/admin" || text === "/help") {
            const helpText = `\u{1F527} <b>ADMIN COMMANDS</b>

<b>\u{1F4F8} Add Track Cover:</b>
...
Example: <code>/list covers</code>`;
          ...
          if (text.startsWith("/list ")) {
            ...
            const data = await env.PORTFOLIO_KV.get(key, "json") || [];
            if (data.length === 0) {
              await sendMessage(chatId, `\u{1F4ED} No items in ${category}`, null, BOT_TOKEN);
            } else {
              let listText = `\u{1F4CB} <b>${category.toUpperCase()}</b> (${data.length} items)
